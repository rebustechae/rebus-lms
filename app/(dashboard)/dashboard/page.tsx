...
import { ArrowRight, Activity, Clock, CheckCircle, Lock, Trophy, BookOpen, Flame } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const greetings = ["Welcome back", "Ready to learn", "Great to see you", "Hello"];
  ...
  const averageProgress = processedCourses.length > 0 
    ? Math.round(processedCourses.reduce((acc, curr) => acc + curr.progressPercent, 0) / processedCourses.length) 
    : 0;

  const CourseCard = ({ course }: { course: any }) => (
    <div className="group relative bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:border-[#00ADEF]/30 transition-all duration-300 flex flex-col h-full">
      {course.is_private && !course.hasStarted && !course.isFulllyCompleted ? (
        <div className="absolute top-6 right-6 text-slate-300"><Lock size={16} /></div>
      ) : course.isFulllyCompleted ? (
        <div className="absolute top-6 right-6 text-emerald-500 bg-emerald-50 p-1 rounded-full"><CheckCircle size={18} /></div>
      ) : null}

      <div className="space-y-4 flex flex-col h-full">
        <div className="flex items-center justify-between">
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-widest ${
            course.isFulllyCompleted ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"
          }`}>
            {course.isFulllyCompleted ? "Completed" : `${course.progressPercent}% Progress`}
          </span>
          <div className="flex items-center gap-1.5 text-slate-400">
            <Clock size={12} />
            <span className="text-[10px] font-bold uppercase tracking-tight">{course.estimatedTime}m</span>
          </div>
        </div>
        ...
        {!course.isFulllyCompleted && (
          <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden mt-4">
            <div className="h-full bg-[#00ADEF] rounded-full transition-all duration-1000" style={{ width: `${course.progressPercent}%` }} />
          </div>
        )}
        ...
        >
          {course.isFulllyCompleted ? "Review" : course.hasStarted ? "Resume" : "Start"}
          <ArrowRight size={16} />
        </Link>
      </div>
    </div>
      ...
      <header className="relative overflow-hidden rounded-sm bg-slate-900 p-8 md:p-12 text-white shadow-2xl">
        <div className="relative z-10 space-y-2">
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight">
            {randomGreeting}, <span className="text-rebus-blue">{displayName}!</span>
          </h1>
          <p className="text-slate-400 text-sm md:text-base max-w-2xl font-medium">
            You've completed {totalLessonsCompleted} lessons so far. Keep pushing toward your next certificate.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white border border-slate-100 p-6 rounded-md shadow-sm flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900">{processedCourses.length}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-md shadow-sm flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-semibold text-slate-400 uppercase">Certificates</p>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-bold text-lg uppercase tracking-wider text-slate-800">In-Progress</h3>
          </div>
        ...
        {activeCourses.length === 0 ? (
          <div className='rounded-[2.5rem] border-2 border-dashed border-slate-200 p-12 text-center bg-slate-50/50'>
            <p className="text-slate-400 text-sm font-medium italic">Your library is currently empty.</p>
            <Link href="/dashboard/courses" className='text-[#00ADEF] text-sm font-bold hover:underline mt-4 inline-flex items-center gap-2'>
      ...
      {completedCourses.length > 0 && (
        <section className="space-y-6">
          <div className="flex items-center gap-3 px-2">
             ...
             <h3 className="font-bold text-lg uppercase tracking-wider text-slate-800">Achievements</h3>
          </div>
